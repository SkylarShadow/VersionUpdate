...
DEFINE_LOG_CATEGORY_STATIC(UnThreadLOG, Log, All);

#define MUTEX_LOCL FScopeLock ScopeLock(&Mutex) 

#define USE_UE_THREAD_POOL_ASYNCTASK(ThreadDelegate) \
(new FAutoDeleteAsyncTask<FUnAbandonable>(ThreadDelegate))->StartBackgroundTask()
...
{FAsyncTask<FUnAbandonable> *UnAbandonable = new FAsyncTask<FUnAbandonable>(ThreadDelegate); \
UnAbandonable->StartBackgroundTask(); \
UnAbandonable->EnsureCompletion(); \
delete UnAbandonable; }
...
#define UN_THREAD_INFO_MSG(Format, ...) \
{ \
	SET_WARN_COLOR(COLOR_CYAN); \
	const FString Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(UnThreadLOG, Log, TEXT("%s"), *Msg); \
	CLEAR_WARN_COLOR(); \
}

#define UN_THREAD_INFO_MSG_WARNING(Format, ...) \
{ \
	SET_WARN_COLOR(COLOR_CYAN); \
	const FString Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(UnThreadLOG, Error, TEXT("%s"), *Msg); \
	CLEAR_WARN_COLOR(); \
}

#define UN_THREAD_INFO_MSG_ERROR(Format, ...) \
{ \
	SET_WARN_COLOR(COLOR_CYAN); \
	const FString Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(UnThreadLOG, Error, TEXT("%s"), *Msg); \
